...
class ManwhaChapter(Base, ManwhaSchema):
    ...
    manwha_id = Column(Integer, ForeignKey('manwha.manwha.id'), primary_key=True, nullable=False)
    chapter_id = Column(Integer, ForeignKey('chapter.chapter.id'), primary_key=True, nullable=False)

class ManwhaAlternativeName(Base, ManwhaSchema):
    __tablename__ = 'manwha_alternative_name'

    manwha_id = Column(Integer, ForeignKey('manwha.manwha.id'), primary_key=True, nullable=False)
    alternative_name_id = Column(Integer, ForeignKey('alternative_name.alternative_name.id'), primary_key=True, nullable=False)
...
class ManwhaAuthor(Base, ManwhaSchema):
    ...
    manwha_id = Column(Integer, ForeignKey('manwha.manwha.id'), primary_key=True, nullable=False)
    author_id = Column(Integer, ForeignKey('author.author.id'), primary_key=True, nullable=False)

class ManwhaArtist(Base, ManwhaSchema):
    __tablename__ = 'manwha_artist'

    manwha_id = Column(Integer, ForeignKey('manwha.manwha.id'), primary_key=True, nullable=False)
    artist_id = Column(Integer, ForeignKey('artist.artist.id'), primary_key=True, nullable=False)
