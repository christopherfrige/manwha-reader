...
class ScrapeKingOfShojoManwhasUseCase(BaseScraperUseCase):
    ...
    def scrape_manwha_chapter_images(self, chapter_url):
        ...
        chapter_images = self.scraper.find_element(By.ID, "readerarea").find_elements(By.TAG_NAME, "img")[1:]
    ...
    def _get_manwha_attributes(self) -> dict:
        ...
        for attribute in manwha_attributes:
            try:
                attribute_name = attribute.find_elements(By.TAG_NAME, "td")[0].get_attribute("innerText")
                attribute_value = attribute.find_elements(By.TAG_NAME, "td")[1].get_attribute("innerText")
    ...
    def _get_manwha_name(self) -> str:
        manwha_name = (
            self.scraper.find_element(By.CLASS_NAME, "entry-title")
    ...
    def _get_chapters_numbers_and_urls(self):
        chapters_raw = self.scraper.find_element(By.ID, "chapterlist").find_elements(By.TAG_NAME, "li")
        chapters = []
        for chapter in chapters_raw:
            ...
            chapter_number = chapter_content.find_element(By.CLASS_NAME, "chapternum").get_attribute("innerText").split('-')[0]

            formatted_chapter_number = float(
                chapter_number[7:].replace(" ", "")
            )
