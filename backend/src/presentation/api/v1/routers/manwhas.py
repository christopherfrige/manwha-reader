...
from src.domain.schemas.manwha import GetManwhasResponse, GetManwhaResponse, SearchManwhasResponse

router = APIRouter(prefix="/v1/manwhas", tags=["v1"])


@router.get("/", response_model=GetManwhasResponse, status_code=200)
def get_manwhas(search: str = "", page: int = 1, per_page: int = 20, db = Depends(UnitOfWork)):
