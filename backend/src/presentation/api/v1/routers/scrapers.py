...
from src.domain.use_cases.scraper.scrape_flower_manwhas import ScrapeFlowerManwhasUseCase
...
@router.post("/flower", status_code=202)
def scrape_flower(
    background_tasks: BackgroundTasks, db=Depends(UnitOfWork), storage=Depends(S3Service)
):
    with db.get_session() as session:
        use_case = ScrapeFlowerManwhasUseCase(session, storage)
        background_tasks.add_task(use_case.execute)
    return {"message": "Scraping request received"}
